...
- [ ] Scenario tabbed navigation — rework `/design/scenarios` into tab-controlled switching between flows/states (like the old HTML mockup navigator) rather than separate routes, so moving between scenario screens mirrors the mockup-switching experience scenarios replace
