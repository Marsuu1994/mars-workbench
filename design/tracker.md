...
- [ ] Scenario pages — `/design/scenarios/*` compose real page components with fixture data as the screen-level source of truth (states hard to reach live). **Board pilot landed** (new-user / returning recap / mid-week / Friday at-risk), its states now tab-switched (`ScenarioTabs`) and each frame `inert` so previews can't fire live actions/navigation. Follow-ups: `scenarios/priorities` (full matrix + tracked + popover) and `scenarios/plan` (form full / review modal all change types / AI chat mid-flow)
