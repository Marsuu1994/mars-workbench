...
- [ ] Scenario pages — `/design/scenarios/*` compose real page components with fixture data as the screen-level source of truth (states hard to reach live). **Board pilot landed**, its states tab-switched (`ScenarioTabs`) to mirror the board mockup's screens (new user / returning recap / board / board — drawer open / mobile backlog sheet); each frame is `inert` (no live actions/navigation) and `contain: layout` so viewport-fixed bits like the mobile "Queued" pill dock inside the frame. The mobile bottom-sheet (a top-layer `<dialog>` that escapes the frame) is shown via a shared `BacklogSheetContent` rendered inline rather than as a modal. Follow-ups: `scenarios/priorities` (full matrix + tracked + popover) and `scenarios/plan` (form full / review modal all change types / AI chat mid-flow)
