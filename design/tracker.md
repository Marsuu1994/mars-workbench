...
- [ ] Scenario pages — `/design/scenarios/*` compose real page components with fixture data as the screen-level source of truth (states hard to reach live). **Board pilot landed**, its states tab-switched (`ScenarioTabs`) to mirror the board mockup's screens (new user / returning recap / board / board — backlog open (desktop) / backlog (mobile)); each frame is `inert` (no live actions/navigation) and `contain: layout` so viewport-fixed bits like the mobile backlog pill dock inside the frame. The mobile backlog (a top-layer `<dialog>` bottom sheet that escapes the frame) is shown via a shared `MobileBacklogContent` rendered inline rather than as a modal. Follow-ups: `scenarios/priorities` (full matrix + tracked + popover) and `scenarios/plan` (form full / review modal all change types / AI chat mid-flow)
- [ ] Gallery tabbed navigation — rework `/design` from the current single long scroll (four zones) into tabs/index that mirror the component layers (`ui/` · `application/` · `domain/`), so the gallery's structure tracks the source tree and each layer is browsable on its own
