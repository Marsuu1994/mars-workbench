...
- **`fx-*` utility layer** in `globals.css` (token-derived via `color-mix`, so both themes restyle automatically): four-corner horizon blooms + star-chart sky on the shell (a fine 24px dot field plus sparse 96px "stars" — replaced the earlier line grid, which read as clutter; mobile keeps only the sparse layer), glass chrome (sidebar; the mobile dock stays solid for scroll perf), reticle corner brackets, `currentColor` console chips, glowing status LEDs, mono telemetry type (`fx-label`/`fx-num` — sans for humans, mono for the machine), glow CTAs, AI holo border, per-quadrant corner blooms, orange drop-target channel. All loops animate opacity/transform only and respect `prefers-reduced-motion`
