...
- **Settings & theme-switch design exploration (pending approval)**: settings leaves its mobile-only page and becomes one responsive overlay — the mobile dock's 4th tab turns trigger (bottom sheet), the desktop sidebar user row turns trigger (centered modal); same `SettingsSheet` on both. Contents: identity card, theme picker (Sora light / Sora dark / P5 dark), two-step confirm sign-out. Flows documented in `design/flows/auth.md` (Settings Overlay / Theme Change / updated Sign-Out); interactive before/after at `design/mockup/future-work/temp-settings-v2.html`
