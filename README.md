...
- **Error-handling spike written** (`design/spike/error-handling.md`, awaiting review): full survey of every data-touching flow's failure behavior (server actions/services/DAL, client hooks/components, page fetches — including the silent optimistic rollbacks, the wedged-spinner class of bugs from missing `.catch`, the zero-error-boundary gap, and the internals-leak paths), then one proposed error language: strictly generic user-facing copy over three surface patterns — auto-dismiss error toast for board/matrix quick ops, one shared inline alert for overlays/forms, and per-segment skeleton → error-screen swap via `error.tsx` — plus two plumbing options (typed error codes over the wire, recommended, vs server-translated strings). Three mars-dark mockups pin all 21 error states: `design/mockup/future-work/mockup-error-{toast,overlay,screen}-v2.html`.
