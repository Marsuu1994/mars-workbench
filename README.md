...
- Plan pages on mobile: desktop header hides, `PlanForm` renders a sticky topbar (create subtitle or the edit plan's week range via a new `periodKey` prop, "Planning" badge), the AI assistant banner compacts to a single tappable row, and the summary + submit footer docks as a fixed bar above the tab bar
