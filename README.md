...
- **Design Console** (`/design`): component gallery tabbed by layer (Tokens & FX · UI · Application · Domain) plus per-feature scenario pages (`/design/scenarios`: board, priorities, plan, AI chat, task modal, auth) rendering the real components with pinned fixture states — the source of truth for implemented UI; unbuilt designs are explored in `design/mockup/future-work/`
...
- **Every feature now has scenario pages** — new `/design/scenarios` pages for **plan** (create / edit / review-changes), **AI chat** (all eight lifecycle states — welcome new/returning, generating, draft, rejected+revised, created, initializing, error — via seeding the chat store), **priorities** (matrix, pinned track popover, no-plan fallback, mobile track sheet ±plan), **task modal** (new/edit template, add-priority-task with and without the quadrant picker), and **auth** (login, mobile settings with the dock); the scenario index lists all six and each page shares one `ScenarioPage` chrome
- **Modal content renders inline in scenario frames** — behavior-neutral extractions split the panel out of the dialog shell: `ReviewChangesPanel`, `TaskModalPanel` (the form now mounts only while the modal is open — same reset-on-open semantics), `AiChatModalContent`, and `LoginScreen` (out of the auth-gated login page); the live modals keep their `OverlayShell` top-layer behavior
