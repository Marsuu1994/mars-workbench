...
- **`p5-dark` ("P5 dark")**: third theme, **full Calling Card skin** — flat-depth geometry fork (halftone shell, sticker cards, hazard targets, parallelogram chips, diamond LEDs, white-flip nav, pop-shadow CTAs with the paper/ink hover flip), Anton display voice on fixed chrome labels, and five opt-in utilities (`fx-display/tile/cut/slash/burst`). Spec `design/design-language/p5-dark.md`
...
- **Undo toast for matrix completion (follow-up PR)** — the "no undo" call was reversed after #34 shipped: completing now shows a 5 s "Marked done" toast (bottom-center on desktop, above the mobile dock) with **Undo** and a countdown bar that drains over the window and holds while hovered; "· +N pts this week" when an active plan absorbed the points. New `ui/Toast` primitive (countdown via the existing `ProgressBar`), which also carries the mobile "Added to …" confirmation so the page has one toast position. Undo write path: `db/tasks.revertAdhocCompletion` (guarded on `status = DONE`, clears `doneAt`, detaches the plan link only if the complete added it), `matrixService.undoCompleteMatrixTask`, `undoCompleteTaskAction` (zod: a client can only ever detach, never attach); the UI waits for the completion write to land before reverting, so an early Undo can't race it. Scenarios: "Done toast" (credited / no plan); gallery: Toast specimens.
...
- **Dump feature shipped (`/kanban/dump`)** — friction-free brain dump, built in two commits.
  ...
  - **UI**: the `/kanban/dump` page + `DumpScreen` (shared BoardHeader + title bar, a pinned composer above a scrolling feed), `DumpComposer` (auto-growing, IME-safe textarea; ⌘/Ctrl+Enter dumps, Enter is a newline — inverse of the AI chat), `DumpFeed` (day-grouped via new `KANBAN_TZ`-anchored helpers in `utils/dump.ts`, IntersectionObserver load-more sentinel, skeletons, empty state), `DumpEntryCard` (6-line clamp + Show more). Feed state is local (`useDumpFeed` hook: optimistic capture with rollback, cursor load-more) — no store, mirroring PriorityMatrixPage. Sidebar link + a 5th mobile dock tab (InboxArrowDownIcon), new `Dump` i18n namespace. Gates green: `tsc --noEmit`, eslint, prettier, `next build`; the date/cursor helpers unit-checked. (Merged as PR #30.)
  - **Design Console**: added the `/design/scenarios/dump` scenario page (Feed / Empty tabs rendering the real `DumpScreen` with fixtures; `initialNextCursor: null` keeps the load-more observer inert), a Dump group in the gallery Domain tab (`DumpEntryCard` short/long specimens + scenario link), and the scenario index row. Retired the exploration mockup `design/mockup/future-work/mockup-dump.html` — the scenario page is now the screen-level source of truth; `flows/dump.md`/`baseline.md`/`tracker.md` reconciled to shipped.

### 2026-07-18

- **`p5-dark` full Calling Card skin shipped**: the gated `[data-theme='p5-dark']` geometry fork lands in `globals.css` — halftone Metaverse shell (no stars/nebula), opaque paper chrome (backdrop blur gone in this theme), paper-bordered panels with hard 8px blood-red offsets, sticker task cards (ink offset → red pop on hover → blood lift on drag), hazard-stripe drop targets, skewed parallelogram chips, diamond LEDs, flat red slash rules, dashed violet AI border with the stop-motion wiggle, flat comet progress arc, 240ms snap-in entrances, white-flip active nav, and red-field CTAs with the paper/ink hover flip. **Anton** joins the font pipeline as the theme's display voice (`fx-display` on chrome labels: board header, column titles, nav/dock labels, modal titles) plus `fx-tile`/`fx-cut`/`fx-slash`/`fx-burst` opt-ins — all inert in the mars themes. Gallery gains a "P5 utilities" section; mars-dark/light are pixel-untouched
...
- **Design Console pass for the settings work (step 2)**: the auth scenario's settings tab now shows **both presentations of the one responsive overlay side by side** — mobile bottom sheet (rest, dock trigger pressed) and desktop centered modal (sign-out pinned in its confirm state); gallery gains `ConfirmButton` (UI tab: rest + triggered) and `ThemePicker` (new Domain → Auth zone); the `/design` theme toggle is now a **three-state cycle button** (Sora dark → Sora light → P5 dark, wraps); `ConfirmButton`'s override prop renamed `armed` → `triggered`; the implemented settings exploration mockup (`temp-settings-v2.html`) retired — the auth scenario is the source of truth
- **Overlay kit slimmed to shell + header**: one new `OverlayHeader` (icon/title/badge cluster + the responsive close affordance — down-chevron on mobile, ✕ from `md` up, carrying the `fx-display` voice) now heads every sheet/modal; `BottomSheet`, the standalone close button, and `TaskModalHeader` are deleted (net −2 components), the `-mx-6` padding hacks are gone (`p-0` boxes + per-region padding, AI-chat gutter normalized to `px-6`), and headers share one type scale. The mobile backlog/track sheets compose `OverlayShell` directly and mount only below `md` via `useBreakpoint` (replacing CSS `mobileOnly`, which could leave the page inert if an open sheet was resized past 768px); gallery and scenario mirrors updated to match
- **Storybook-vs-Design-Console spike written** (`design/spike/design-console-vs-storybook.md`, awaiting review): pain points (panel/shell split, no true breakpoint simulation in scenario frames), honest pros/cons for keeping the in-app console vs migrating, and a phased implementation plan for each; `design/spike/` is now the documented home for open-question spikes (AGENTS.md + design README)
...
- **Journal design migrated to the scenario-first workflow** — merged the mockup retirement into the journal design branch: the exploration mockup moved to `design/mockup/future-work/mockup-journal.html` and is now fully self-contained (theme tokens + the used subset of the retired shared mockup CSS inlined; both themes re-verified via headless-Chromium screenshots), and every design-doc pointer (baseline, flows, tracker, README) re-points there. Approval flow unchanged: on approval V1 is implemented, `/design/scenarios/journal` takes over, and the exploration mockup is deleted
- **Settings & theme-switch design exploration (pending approval)**: settings leaves its mobile-only page and becomes one responsive overlay — the mobile dock's 4th tab turns trigger (bottom sheet), the desktop sidebar user row turns trigger (centered modal); same `SettingsSheet` on both. Contents: identity card, theme picker (Sora light / Sora dark / P5 dark), two-step confirm sign-out. Side-effecting flows documented in `design/flows/auth.md` (Theme Change / updated Sign-Out; the overlay UI itself is documented by the mockup); interactive before/after at `design/mockup/future-work/temp-settings-v2.html`
- **New copy convention (AGENTS.md)**: user-facing copy is sentence case; Title Case reserved for headers — fixes the "Sign Out" vs "Sign out?" drift class
- **Design docs restructured into `design/design-language/`** — one folder, one md per theme (`mars-dark` / `mars-light` / `p5-dark`) plus a README carrying the shared skeleton (fx-* utility API, voices, motion, contrast method, sync points). Replaces the two-file split (`design-system.md` + `design-system-p5.md`); the P5 doc is reframed from "mars-dark reskin" to what it now is — the spec for the third theme, gated under `[data-theme='p5-dark']`, mars themes untouched
- **Journal simplified to "Dump" (pending approval)** — per owner review the feature is renamed and cut to its core: capture + view, storage-only. All LLM/categorization design is removed (no categories, no background classification, no ✦ chips, no recategorize picker); the schema collapses to a single `DumpEntry` table (`content`, `createdAt`, plus a dormant `isProcessed` flag reserved for a future LLM batch-processing flow, TBD). Docs follow: `flows/journal.md` → `flows/dump.md` (5 flows → 2), baseline entities/schema/architecture trimmed to match, tracker section rewritten (`## Journal` → `## Dump`), and the exploration mockup reworked + renamed `mockup-journal.html` → `mockup-dump.html` (screens: Dump / Empty / Mobile ×2 phones, "Dump" nav item with inbox icon; both themes re-verified via headless-Chromium screenshots)
