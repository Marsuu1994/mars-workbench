...
- **One-off tasks on the board**: board cards show one-off (`AD_HOC`) tasks with an amber "ONCE" type badge (the internal type stays `AD_HOC`; all user-facing "ad-hoc" copy reads "one-off"). Created from the priority matrix only (board-side creation removed) and reach the board via Track This Week; risk levels based on days since creation apply to board cards only — see [priorities/README.md](../priorities/README.md)
